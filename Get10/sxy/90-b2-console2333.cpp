...
{
	int sbx, sby, sbxingwei, jp1, jp2, fs, X = 1, Y = 1;
	char zb[80];
	...
	while (1) {
		...
		if (fs == 1) {//键盘
			if (jp1 == 224) {
				...
				cct_showch(a, b, ' ', 0, 7, 60);//清除一开始那句话
				cct_gotoxy(a, b);
...
{
	int i, j, fs, sbx, sby, sbxingwei, jp1, jp2, t = 300;
	cct_enable_mouse();//可以用鼠标
	cct_setcursor(CURSOR_INVISIBLE);//光标不显示
	...
	if ((fs == 1 && jp1 == 13 )|| (fs == 0 && sbxingwei == MOUSE_LEFT_BUTTON_CLICK)) {
		hebing(zb, sz, jl, x0, y0, n);//标记’1‘，且加一，有*的地方变成’0‘
		...
		for (i = 0; i < x0; i++) {
			for (j = 0; j < y0; j++) {
				if (jl[i + 1][j + 1] == '*')
					...
					biansai(i + 1, j + 1, sz, 7, 0);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1bo传参为0就不重复加一了
				}
			}
		}
		cct_showch(a, b, ' ', 0, 7, 60);//清除一开始那句话
		cct_gotoxy(a, b);
		cout << "！！！！！！！！！！！！！！！！！！！！"<<endl<<"a gift for your wonderful fantastic performance";
		getchar();
		xialuotuxing(sz, jl, x0, y0);
		getchar();

		tianchong(sz, jl, 5, x0, y0, '8', zhaozuida(sz));
