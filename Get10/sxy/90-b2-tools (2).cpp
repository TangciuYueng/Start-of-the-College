...
void shuchuxh(char zb[],char sz[N][M],char jl[N][M],char ch, int x0, int y0,char n,int mb) {//输出带星号的数组
	int i, j;
	cout << "查找结果数组：" << endl;
	cout << "  |";
...
void shangsai(char zb[],char sz[N][M], char jl[N][M],char ch, int x0, int y0,char n,int mb,bool bo)//上色
{
	int i, j, a, b;
	char qr;
	if (bo)
